...
export default async function LessonPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  try {
    lesson = await loadLessonContentJSON("excel", lessonId);
  } catch (error) {
  ...
  if (!lesson) {
    ...
    try {
      const availableLessons = await loadAllLessonsJSON("excel");
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex items-center space-x-6">
              <div className="flex items-center text-sm text-gray-600 bg-gray-100 px-3 py-2 rounded-full">
                ...
                {lesson.metadata?.estimatedTime || lesson.metadata?.duration || '25 minutes'}
              </div>
