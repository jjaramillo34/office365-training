...
export default async function LessonPage({ params }: { params: Promise<{ lessonId: string }> }) {
  ...
  let lesson;
  try {
    lesson = await loadLessonContent("excel", lessonId);
  } catch (error) {
    console.error(`Error loading lesson content for excel/${lessonId}:`, error);
    lesson = null;
  }
  
  if (!lesson) {
    console.error(`Lesson not found for excel/${lessonId}`);
    try {
      const availableLessons = await loadAllLessons("excel");
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6">
                ...
                <nav className="space-y-2">
                  {sections.map((section, index) => (
                    <div
                      key={index}
                      className="w-full text-left px-4 py-3 rounded-xl text-sm text-gray-700 hover:bg-green-50 hover:text-green-700 transition-all duration-200 cursor-pointer border border-transparent hover:border-green-200"
                    >
                      <div className="flex items-center">
                        <div className="w-6 h-6 bg-green-100 rounded-full flex items-center justify-center mr-3 text-xs font-bold text-green-600">
                          {index + 1}
                        </div>
          ...
          <div className="lg:col-span-3">
                ...
                <div key={index} className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                  <div className="bg-gradient-to-r from-green-50 to-emerald-50 px-8 py-6 border-b border-gray-100">
                    <h2 className="text-2xl font-bold text-gray-900 flex items-center">
                      ...
                      {!section.title.includes('Best Practices') && !section.title.includes('Troubleshooting') && !section.title.includes('Shortcuts') && !section.title.includes('Tips') && !section.title.includes('Exercise') && <BookOpen size={24} className="mr-3 text-green-600" />}
                      {section.title}
                    </h2>
                  </div>
                  <div className="p-8">
                        ...
                        components={{
                          ...
                          h1: ({ children, id }) => (
                            <h1 id={id} className="text-3xl font-bold text-gray-900 mb-6 mt-8 border-b border-gray-200 pb-4 group">
                              {children}
                              {id && (
                                <a href={`#${id}`} className="ml-2 opacity-0 group-hover:opacity-100 transition-opacity text-green-500 hover:text-green-700">
                                  #
                                </a>
                              )}
                            </h1>
                          ...
                          blockquote: ({ children }) => (
                            <blockquote className="border-l-4 border-green-500 bg-green-50 pl-6 py-4 my-6 rounded-r-xl italic">
                              {children}
                            </blockquote>
                          ),
                          ...
                          a: ({ href, children }) => {
                            ...
                            return (
                              <a 
                                href={href} 
                                className="text-green-600 hover:text-green-800 underline inline-flex items-center"
            ...
            <div className="mt-12 flex justify-between items-center">
              ...
              <Link
                href="/excel/quiz"
                className="inline-flex items-center px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-green-600 to-emerald-600 border border-transparent rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-lg"
              >
                Take Quiz
